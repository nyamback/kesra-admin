         ...
         <form action="{{route('products.update',$item->id)}}" method="POST">
            ...
            <div class="form-group col-2 p-0">
                ...
                <select name="type" class="form-control @error('type') is invalid @enderror">
                    ...
                    {{-- <option value="desain" {{old('type', $item->type) == "desain" ? 'selected' : ''}}>desain</option>
                    <option value="foto&video" {{old('type', $item->type) == "foto&video" ? 'selected' : ''}}>foto&video</option>
                    <option value="reparasi" {{old('type', $item->type) == "reparasi" ? 'selected' : ''}}>reparasi</option> --}}
                </select>
                @error('type')
                    <div class="text-danger">{{$message}}</div>
                @enderror
            ...
            <div class="form-group col-2 pt-3 pl-0 text-left">
                <button class="btn btn-primary btn-block" type="submit">Update Produk</button>
            </div>
